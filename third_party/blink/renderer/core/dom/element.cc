...
Attr* Element::DetachAttribute(wtf_size_t index, ExceptionState& exception_state) {
  DCHECK(GetElementData());
  const Attribute& attribute = GetElementData()->Attributes().at(index);
  Attr* attr_node = AttrIfExists(attribute.GetName());

  bool allowed;
  probe::WillModifyDOMAttrExtended(this, attribute.GetName(), attr_node ? attr_node->value() : g_null_atom, g_null_atom, allowed);
  if (!allowed) {
    exception_state.ThrowDOMException(
      DOMExceptionCode::kNotAllowedError,
      "Attribute modification blocked by DOMGuard.");
    return nullptr;
  }
...
void Element::removeAttribute(const QualifiedName& name, ExceptionState& exception_state) {
  if (!GetElementData())
    return;
  ...
  bool allowed;
  const Attribute& attribute = GetElementData()->Attributes().at(index);
  probe::WillModifyDOMAttrExtended(this, name, attribute.Value(), g_null_atom, allowed);
  if (!allowed) {
    exception_state.ThrowDOMException(
      DOMExceptionCode::kNotAllowedError,
      "Attribute modification blocked by DOMGuard.");
...
static bool isAttached(Node *node) {
  do {
...
bool Element::toggleAttribute(const AtomicString& qualified_name,
  ...
  if (!GetAttributeHinted(lowercase_name, hint)) {
    ...
    SetAttributeHinted(lowercase_name, hint, g_empty_atom, exception_state);
...
bool Element::toggleAttribute(const AtomicString& qualified_name,
  ...
  // qualifiedName and the context object, and then return false.
  if (!force) {
    RemoveAttributeHinted(lowercase_name, hint, exception_state);
    return exception_state.HadException();
  }
  // 6. Return true.
  return true;
...
void Element::ParserSetAttributes(const Vector<Attribute>& attribute_vector, bool is_parsing_fragment) {
  DCHECK(!isConnected());
  DCHECK(!parentNode());
  ...
  bool should_delay_attribute_changed = (is_parsing_fragment && GetDocument().domWindow());
  ...
  for (const auto& attribute : attribute_vector) {
    if (!should_delay_attribute_changed) {
    ...
    } else if (attribute.GetName() == "id" || attribute.GetName() == "class") {
      AttributeChanged(AttributeModificationParams(
        attribute.GetName(), g_null_atom, attribute.Value(),
        AttributeModificationReason::kByParser));
    } else if (DynamicTo<HTMLCanvasElement>(this) && (attribute.GetName() == "width" || attribute.GetName() == "height")) {
      AttributeChanged(AttributeModificationParams(
        attribute.GetName(), g_null_atom, attribute.Value(),
        AttributeModificationReason::kByParser));
    } else if (tagName() == "INPUT" && attribute.GetName() == "type") { // Cannot DynamicTo<HTMLInputElement>
      AttributeChanged(AttributeModificationParams(
        attribute.GetName(), g_null_atom, attribute.Value(),
        AttributeModificationReason::kByParser));
    } else {
      EnsureUniqueElementData().pending_attribute_modifications_.push_back(
        UniqueElementData::AttributeModificationParams(attribute.GetName(), g_null_atom, attribute.Value(), 1));
    }
  }
}
...
scoped_refptr<ComputedStyle> Element::StyleForLayoutObject(
  ...
  scoped_refptr<ComputedStyle> style =
      HasCustomStyleCallbacks()
          ? CustomStyleForLayoutObject(style_recalc_context, allowed)
          : OriginalStyleForLayoutObject(style_recalc_context, allowed);
  if (!allowed) {
    return nullptr;
  }
  if (!style) {
...
StyleRecalcChange Element::RecalcOwnStyle(
  ...
  if (ParentComputedStyle()) {
    ...
    if (!new_style)
      new_style = StyleForLayoutObject(style_recalc_context, allowed);
...
Attr* Element::removeAttributeNode(Attr* attr,
  ...
  bool allowed;
  probe::WillModifyDOMAttrExtended(this, attr->GetQualifiedName(), attr->value(), g_null_atom, allowed);
  if (!allowed) {
    exception_state.ThrowDOMException(
      DOMExceptionCode::kNotAllowedError,
      "Attribute modification blocked by DOMGuard.");
...
void Element::RemoveAttributeInternal(
  ...
  if (!in_synchronization_of_lazy_attribute)
    DidRemoveAttribute(name, value_being_removed, delay_attribute_changed);
}
...
void Element::AppendAttributeInternal(
  ...
  if (!in_synchronization_of_lazy_attribute)
    DidAddAttribute(name, value, delay_attribute_changed);
}
  ...
  if (!element_style || element_style->IsEnsuredOutsideFlatTree()) {
    ...
    if (!element_style) {
      ...
      if (HasCustomStyleCallbacks() && !IsA<HTMLImageElement>(*this))
        new_style = CustomStyleForLayoutObject(style_recalc_context, allowed);
      else
        new_style = OriginalStyleForLayoutObject(style_recalc_context, allowed);
...
void Element::UpdateFirstLetterPseudoElement(StyleUpdatePhase phase) {
  ...
  if (phase == StyleUpdatePhase::kAttachLayoutTree) {
    ...
    scoped_refptr<ComputedStyle> pseudo_style =
        element->StyleForLayoutObject(style_recalc_context, allowed);
...
PseudoElement* Element::CreatePseudoElementIfNeeded(PseudoId pseudo_id) {
  ...
  bool allowed = true;
  scoped_refptr<ComputedStyle> pseudo_style =
      pseudo_element->StyleForLayoutObject(style_recalc_context, allowed);
  if (!PseudoElementLayoutObjectIsNeeded(pseudo_style.get(), this)) {
    GetElementRareData()->SetPseudoElement(pseudo_id, nullptr);
    return nullptr;
  }
  ...
  if (allowed && RuntimeEnabledFeatures::LayoutNGEnabled())
...
const ComputedStyle* Element::CachedStyleForPseudoElement(
  ...
  bool allowed = true;
  scoped_refptr<ComputedStyle> result = StyleForPseudoElement(request, allowed, style);
  if (allowed && result)
...
scoped_refptr<ComputedStyle> Element::StyleForPseudoElement(
  ...
  if (is_before_or_after) {
    ...
    return GetDocument().GetStyleResolver().PseudoStyleForElement(
        this, allowed, request, style, layout_parent_style);
  }
  ...
  if (request.pseudo_id == kPseudoIdFirstLineInherited) {
    ...
    if (IsPseudoElement()) {
      ...
      result = GetDocument().GetStyleResolver().StyleForElement(
          this, allowed, style_recalc_context, parent_style, parent_style);
    }
    if (result)
  ...
  return GetDocument().GetStyleResolver().PseudoStyleForElement(
      this, allowed, request, parent_style, parent_style);
}
...
void Element::DidAddAttribute(const QualifiedName& name,
  ...
  bool should_delay_attribute_changed = (delay_attribute_changed && !isAttached(this) && GetDocument().domWindow());
  if (!should_delay_attribute_changed) {
    AttributeChanged(AttributeModificationParams(
      name, g_null_atom, value, AttributeModificationReason::kDirectly));
  } else if (name.LocalName() == "id" || name.LocalName() == "class") {
    AttributeChanged(AttributeModificationParams(
      name, g_null_atom, value, AttributeModificationReason::kDirectly));
  } else if (DynamicTo<HTMLCanvasElement>(this) && (name.LocalName() == "width" || name.LocalName() == "height")) {
    AttributeChanged(AttributeModificationParams(
      name, g_null_atom, value, AttributeModificationReason::kDirectly));
  } else if (tagName() == "INPUT" && name.LocalName() == "type") { // Cannot DynamicTo<HTMLInputElement>
    AttributeChanged(AttributeModificationParams(
      name, g_null_atom, value, AttributeModificationReason::kDirectly));
  } else {
    EnsureUniqueElementData().pending_attribute_modifications_.push_back(
      UniqueElementData::AttributeModificationParams(name, g_null_atom, value, 0));    
  }
...
void Element::DidModifyAttribute(const QualifiedName& name,
  ...
  if (!should_delay_attribute_changed) {
    ...
    EnsureUniqueElementData().pending_attribute_modifications_.push_back(
      UniqueElementData::AttributeModificationParams(name, old_value, new_value, 0));    
  }
...
void Element::DidRemoveAttribute(const QualifiedName& name,
  ...
  if (!should_delay_attribute_changed) {
    ...
    EnsureUniqueElementData().pending_attribute_modifications_.push_back(
      UniqueElementData::AttributeModificationParams(name, old_value, g_null_atom, 0));     
  }
...
scoped_refptr<ComputedStyle> Element::CustomStyleForLayoutObject(
    ...
    bool &allowed) {
  DCHECK(HasCustomStyleCallbacks());
  return OriginalStyleForLayoutObject(style_recalc_context, allowed);
}
...
void Element::CloneAttributesFrom(const Element& other) {
  ...
  bool should_delay_attribute_changed = (GetDocument().domWindow() != nullptr);
  for (const Attribute& attr : element_data_->Attributes()) {
    if (!should_delay_attribute_changed) {
      AttributeChanged(AttributeModificationParams(
        attr.GetName(), g_null_atom, attr.Value(),
        AttributeModificationReason::kByCloning));
    } else if (attr.GetName() == "id" || attr.GetName() == "class") {
      AttributeChanged(AttributeModificationParams(
        attr.GetName(), g_null_atom, attr.Value(),
        AttributeModificationReason::kByCloning));
    } else if (DynamicTo<HTMLCanvasElement>(this) && (attr.GetName() == "width" || attr.GetName() == "height")) {
      AttributeChanged(AttributeModificationParams(
        attr.GetName(), g_null_atom, attr.Value(),
        AttributeModificationReason::kByCloning));
    } else if (tagName() == "INPUT" && attr.GetName() == "type") { // Cannot DynamicTo<HTMLInputElement>
      AttributeChanged(AttributeModificationParams(
        attr.GetName(), g_null_atom, attr.Value(),
        AttributeModificationReason::kByCloning));
    } else {
      EnsureUniqueElementData().pending_attribute_modifications_.push_back(
        UniqueElementData::AttributeModificationParams(attr.GetName(), g_null_atom, attr.Value(), 2));
    }
  }
...
void Element::ExecutePendingAttributeChanges() {
  if (GetElementData() && GetElementData()->IsUnique() && EnsureUniqueElementData().pending_attribute_modifications_.size()) {
    for (const auto& params : EnsureUniqueElementData().pending_attribute_modifications_) {
      AttributeChanged(AttributeModificationParams(
        params.name, params.old_value, params.new_value,
        params.reason == 0 ? 
    ...
    EnsureUniqueElementData().pending_attribute_modifications_.clear();
  }
}
