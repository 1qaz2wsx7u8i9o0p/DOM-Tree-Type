...
void Element::setAttribute(const QualifiedName& name,
  ...
  if (index == kNotFound) {
    probe::WillModifyDOMAttrExtended(this, name, g_null_atom, value, allowed);
  } else {
    const Attribute& attribute = GetElementData()->Attributes().at(index);
    probe::WillModifyDOMAttrExtended(this, name, attribute.Value(), value, allowed);
  }
...
void Element::SetAttributeHinted(const AtomicString& local_name,
  ...
  if (index == kNotFound) {
    probe::WillModifyDOMAttrExtended(this, q_name, g_null_atom, value, allowed);
  } else {
    const Attribute& attribute = GetElementData()->Attributes().at(index);
    probe::WillModifyDOMAttrExtended(this, q_name, attribute.Value(), value, allowed);
  }
  
  if (!allowed) {
    exception_state.ThrowDOMException(
      DOMExceptionCode::kNotAllowedError,
      "Attribute modification blocked by DOMGuqard.");
    return;
  }
...
Attr* Element::setAttributeNode(Attr* attr_node,
  ...
  bool allowed;
  probe::WillModifyDOMAttrExtended(this, attr_node->GetQualifiedName(), old_attr_node ? old_attr_node->value() : g_null_atom, value, allowed);
  if (!allowed) {
    exception_state.ThrowDOMException(
      DOMExceptionCode::kNotAllowedError,
      "Attribute modification blocked by DOMGuard.");
...
void Element::RemoveAttributeHinted(const AtomicString& name,
  ...
  wtf_size_t index;
  QualifiedName q_name = QualifiedName::Null();
  std::tie(index, q_name) = LookupAttributeQNameHinted(name, hint);

  bool allowed;

  if (index == kNotFound) {
    probe::WillModifyDOMAttrExtended(this, q_name, g_null_atom, g_null_atom, allowed);
  } else {
    const Attribute& attribute = GetElementData()->Attributes().at(index);
    probe::WillModifyDOMAttrExtended(this, q_name, attribute.Value(), g_null_atom, allowed);
  }
