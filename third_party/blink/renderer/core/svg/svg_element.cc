...
scoped_refptr<ComputedStyle> SVGElement::CustomStyleForLayoutObject(
    const StyleRecalcContext& style_recalc_context, bool& allowed) {
  ...
  if (!corresponding_element) {
    return GetDocument().GetStyleResolver().StyleForElement(
        this, allowed, style_recalc_context);
  }
  ...
  return GetDocument().GetStyleResolver().StyleForElement(
      corresponding_element, allowed, style_recalc_context, style, style);
}
