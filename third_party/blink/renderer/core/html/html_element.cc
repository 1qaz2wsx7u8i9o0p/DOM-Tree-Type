...
void HTMLElement::setInnerText(const String& text,
  ...
  if (!text.Contains('\n') && !text.Contains('\r')) {
    if (text.IsEmpty()) {
      bool allowed = RemoveChildrenExtended();
      if (!allowed) {
...
void HTMLElement::setContentEditable(const String& enabled,
  ...
  if (EqualIgnoringASCIICase(enabled, "true"))
    setAttribute(html_names::kContenteditableAttr, AtomicString("true"), exception_state);
  else if (EqualIgnoringASCIICase(enabled, "false"))
    setAttribute(html_names::kContenteditableAttr, AtomicString("false"), exception_state);
  else if (EqualIgnoringASCIICase(enabled, "plaintext-only"))
    setAttribute(html_names::kContenteditableAttr, AtomicString("plaintext-only"), exception_state);
  else if (EqualIgnoringASCIICase(enabled, "inherit"))
    removeAttribute(html_names::kContenteditableAttr, exception_state);
  else
