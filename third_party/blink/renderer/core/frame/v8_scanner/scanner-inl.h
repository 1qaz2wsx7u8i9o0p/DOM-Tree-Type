...
#include "third_party/blink/renderer/core/frame/v8_scanner/keywords-gen.h"
...
Token::Value KeywordOrIdentifierToken(const uint8_t* input,
                                                int input_length) {
...
Token::Value Scanner::ScanIdentifierOrKeywordInner() {
  bool escaped = false;
  bool can_be_keyword = true;

  if (static_cast<uint32_t>(c0_) <= kMaxAscii) {
    if (c0_ != '\\') {
      ...
      if (!IdentifierNeedsSlowPath(scan_flags)) {
        ...
        return KeywordOrIdentifierToken(&*chars.begin(), chars.size());
      }
...
Token::Value Scanner::ScanSingleToken() {
  ...
  do {
    ...
    if (static_cast<unsigned>(c0_) <= kMaxAscii) {
